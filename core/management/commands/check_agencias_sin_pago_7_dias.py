...
class Command(BaseCommand):
    help = "Enviar correo a agencias con pagos pendientes que superan 7 dias"
    ...
    def handle(self, *args, **kwargs):
        ...
        reservas_pendientes = (
            Reserva.objects.filter(fecha_reserva__date__lte=limite)
        ...
        for email, reservas in agrupadas.items():
            try:
                ...
                subject = f"Pago pendiente: reservas sin pago > 7 dias ({hoy.strftime('%d/%m/%Y')})"
                ...
                body = (
                    "Hola,\n\n"
                    "Estas reservas de agencia superaron 7 dias sin pago:\n"
                    f"{detalle_txt}\n\n"
                ...
                logger.error("Fallo enviando recordatorio 7 dias a %s: %s", email, e)

        self.stdout.write(self.style.SUCCESS(f"Se enviaron {enviados} recordatorios de 7 dias."))
